...
export class MedicoRepository{
    ...
    async getMedicoCrm(crm?:number): Promise<Medico> {
        let query = "SELECT * FROM interdisciplinar.medico WHERE crm = ?";
        try {
            const resultado = await executarComandoSQL(query, [crm]);
            console.log('Pessoa localizada com sucesso, ID: ', resultado);
            return new Promise<Medico>((resolve)=>{
                resolve(resultado);
            })
        } catch (err:any) {
            console.error(`Falha ao procurar pessoa de ID ${crm} gerando o erro: ${err}`);
            throw err;
        }    
    }
