...
export class ExameRepository{
    ...
    async insereExame(exame: Exame):Promise<Exame>{
        try {
            const resultado = await executarComandoSQL(
                "INSERT INTO interdisciplinar.exame (pacienteCpf, medicoCrm, data, pacienteNome, medicoNome) VALUES (?,?,?,?,?)",
                [exame.pacienteCpf, exame.medicoCrm, exame.data, exame.pacienteNome, exame.medicoNome]
            );
            exame.id = resultado.insertId;
            console.log('exame criado com sucesso:', exame);
            return exame;
        } catch (err) {
    ...
    async getExameCpf(cpf?:string): Promise<Exame[]> {
