...
export class ExameService{
    ...
    async novoExame(exameData: any):Promise<Exame>{
        ...
        if(paciente && medico){
            ...
            const examesEncontrados: Exame[]= await this.exameRepository.getExameCpf(exame.pacienteCpf)

            if(examesEncontrados.length > 0){
            throw new Error("Já existe um Exame cadastrado pra este paciente");
            }
            
            return this.exameRepository.insereExame(exame);
        }
        else{
